...
/**
 * Interface Class 接口类需要2个参数 参数1: 接口的名字 (string) 参数2: 接受方法名称的集合(数组) (array)
 */
BH.Interface = function(name, methods) {
	// 判断接口的参数个数
	if (arguments.length != 2) {
		throw new Error(
				'this instance interface constructor arguments must be 2 length!');
	}
	this.name = name;
...
BH.Interface.ensureImplements = function(object) {
	...
	if (arguments.length < 2) {
		throw new Error(
				'Interface.ensureImplements method constructor arguments must be  >= 2!');
	}
